    ...
    if (uniqueRestaurantIds.length === 1 && uniqueRestaurantIds[0] === userRestaurantId) {
      ...
      if (foreignUsers.length > 0) {
        ...
        foreignUsers.forEach(u => {
          console.log(`  - ${u.full_name} (${u.email}) from restaurant ${u.restaurant_id}`);
        });
      }
    }
      ...
      try {
        ...
        if (userDetailResponse.data && userDetailResponse.data.data) {
          ...
          if (userDetail.restaurant_id === userRestaurantId) {
            console.log(`✅ User ${userDetail.full_name}: Access allowed (same restaurant)`);
          } else {
            console.log(`❌ User ${userDetail.full_name}: SECURITY BREACH (different restaurant: ${userDetail.restaurant_id})`);
          }
        }
      } catch (error) {
        if (error.response && error.response.status === 403) {
          console.log(`✅ User ${userId}: Access properly denied (403 Forbidden)`);
        } else {
