...
async function testRestaurantIdSaving() {
  try {
    // Login first
    ...
    // Create user data WITHOUT restaurant_id (should be auto-assigned)
    const userData = {
      full_name: 'Test Restaurant ID User',
      email: '',
      password: '12345678',
      role_location_pairs: [
        {
    ...
    console.log('\n=== Creating User Without Restaurant ID ===');
    console.log('User data (no restaurant_id):', JSON.stringify(userData, null, 2));
    ...
    if (createResponse.status === 201) {
      ...
      console.log('Created user data:', JSON.stringify(createResult.data, null, 2));
      console.log('🔍 Restaurant ID saved:', createResult.data.restaurant_id);

      if (createResult.data.restaurant_id === loginData.user.restaurant_id) {
        console.log('✅ Restaurant ID correctly auto-assigned from current user!');
      } else {
        console.log('❌ Restaurant ID mismatch!');
        console.log('Expected:', loginData.user.restaurant_id);
